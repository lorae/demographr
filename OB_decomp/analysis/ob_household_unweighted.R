...
hh_2000 <- generate_households(
  ...
  filter(subfamily_id == 1) |>
  mutate(
    n_nonsf = hh_size - subfamily_size,
    age = subfam_head_age,
    n_spouses = n_partners # TODO: change name upstream
    )
...
hh_2019 <- generate_households(
  ...
  mutate(
    n_nonsf = hh_size - subfamily_size,
    age = subfam_head_age,
    n_spouses = n_partners
  )

# ----- Step 4: Apply KOB ---- #

hhsize_2000 <- hh_2000$hh_size |> mean()
hhsize_2019 <- hh_2019$hh_size |> mean()

age_2000 <- hh_2000$age |> mean()
age_2019 <- hh_2019$age |> mean()

children_2000 <- hh_2000$n_children |> mean()
children_2019 <- hh_2019$n_children |> mean()

spouse_2000 <- hh_2000$n_spouses |> mean()
spouse_2019 <- hh_2019$n_spouses |> mean()

nonsf_2000 <- hh_2000$n_nonsf |> mean()
nonsf_2019 <- hh_2019$n_nonsf |> mean()
...
reg_children_2000 <- lm(n_children ~ age, data = hh_2000)
reg_children_2019 <- lm(n_children ~ age, data = hh_2019)

# Spouse regressions
reg_spouse_2000 <- lm(n_spouses ~ age, data = hh_2000)
reg_spouse_2019 <- lm(n_spouses ~ age, data = hh_2019)

# Non-subfamily regressions
reg_nonsf_2000 <- lm(n_nonsf ~ age, data = hh_2000)
reg_nonsf_2019 <- lm(n_nonsf ~ age, data = hh_2019)
